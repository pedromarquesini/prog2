...
void adicionaLesaoPaciente(Paciente* pct){
    pct->qtdLesoes++;
    //alocar espaço para lesao

    Lesao *les = NULL;
    les = lesao_alloc(les);

    //cadastrar lesao com funcao lesao_cadastraLesao
    les = lesao_cadastro(les);

    //alocar espaço para a lesao no vetor de lesões do paciente, usando funcao externa realloc
    lesao_realloc(pct->lesoes, pct->qtdLesoes);
    
    //adicionar lesao no vetor de lesões do paciente
    lesao_addLesao(pct->lesoes, les, pct->qtdLesoes);
